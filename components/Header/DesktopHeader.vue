<template>
  <header ref="header" class="fixed top-0 w-full z-50 py-2 px-8 flex justify-between items-center dark:bg-[#09090B]/[75%]
   bg-[#FAFAFA]/[75%]  border-[#000]/[0.1] dark:border-[#fff]/[0.1] backdrop-blur-[8px]">
    <div class="max-w-[1440px] mx-auto flex justify-between w-full">
     ...
     <div class="flex justify-start items-center flex-1">
      ...
      <div class="flex items-center justify-center flex-1">
        <nav class="text-[14px] w-full rounded-2xl space-y-[6px] px-[4px] py-[6px]
         bg-[#000]/[0.2] dark:bg-[#fff]/[0.2] backdrop-blur-[4px]">
         ...
         <div class="relative flex justify-center gap-2">
            ...
            <div class="flex-1" @mouseenter="handleMouseEnter(0)" @mouseleave="handleMouseLeave"> 
              ...
              class="nav-link block text-center cursor-pointer text-[#09090B] dark:text-[#FAFAFA]">
              {{ $t('header.home') }}
              </NuxtLink>
            </div>
            ...
            <span :style="{ left: (25 * activeLinkIndex) + '%', width: '25%' }" 
            class="absolute bottom-0 h-full scale-x-100 scale-y-125 rounded-xl bg-[#ffffff]
             dark:bg-[#000000] transition-all duration-300">
            </span>
      ...
      <div class="flex justify-end flex-1 items-center">
       <div class="menu-container relative">
        ...
        <nav class="flex items-center">
          ...
          <a target="_blank">
            <button class="inline-flex justify-center whitespace-nowrap rounded-md px-0 
            p-2 h-9 w-9 hover:bg-[#F4F4F5] dark:hover:bg-[#27272A] transition-colors">
              <ContactIcon class="w-[1.2rem] h-[1.2rem] fill-black dark:fill-white hover:scale-105" />
            </button>
          </a>

          <a>
            <button @click="toggleMenu" class="menu-toggle-button inline-flex justify-center whitespace-nowrap 
            rounded-md px-0 p-2 h-9 w-9 hover:bg-[#F4F4F5] dark:hover:bg-[#27272A] transition-colors">
        ...
        <div v-if="menuOpen" class="absolute right-0 top-10 mt-2 p-1 w-32 rounded-md dark:bg-[#09090B]
         bg-[#ffffff] border border-[#000]/5 dark:border-[#fff]/5 backdrop-blur-[8px] z-50">

          <button @click="setTheme('light')" class=" rounded-sm block px-4 py-2 text-sm text-[#09090B]
           dark:text-[#FAFAFA] w-full text-left hover:bg-[#F4F4F5] dark:hover:bg-[#27272A]">
           {{ $t('header.light') }}
          </button>

          <button @click="setTheme('dark')" class="rounded-sm block px-4 py-2 text-sm text-[#09090B]
           dark:text-[#FAFAFA] w-full text-left hover:bg-[#F4F4F5] dark:hover:bg-[#27272A]">
           {{ $t('header.dark') }}
          </button>
           
          <button @click="setTheme('system')" class="rounded-sm block px-4 py-2 text-sm text-[#09090B]
           dark:text-[#FAFAFA] w-full text-left hover:bg-[#F4F4F5] dark:hover:bg-[#27272A]">
           {{ $t('header.system') }}
          </button>
